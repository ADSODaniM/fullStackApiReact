...
// Componente funcional InvoicePDF
const InvoicePDF = () => {
  const location = useLocation(); // Hook para acceder al estado de la navegación
  // Destructurar los datos del estado de la navegación
  ...
  const generatePDF = () => {
    ...
    doc.text('Nail Salon', 105, 30, null, null, 'center');
    doc.text('Carrera 55A Nº. 52-82', 105, 35, null, null, 'center');
    doc.text('Teléfono: 8336515', 105, 40, null, null, 'center');
    doc.text('Correo: nailsalon@correo.com', 105, 45, null, null, 'center');
  ...
  return (
    <div className="payment-form" style={{ textAlign: 'left' }}>
      ...
      <button className='test' onClick={generatePDF}>Descargar Factura en PDF</button>
      <button className='test' onClick={() => alert('Proceder al pago PSE o en efectivo')}>Proceder al Pago</button>
    </div>
  );
};

// Exporta el componente InvoicePDF como la exportación predeterminada del módulo
